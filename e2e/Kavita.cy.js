...
describe('Home Page', function () {
   ...
   // home page slider , click on second slider
    ...
    it('Product click from the Slider', () => { //prev
        cy.wait(5000)
        cy
        .get('.slick-track')
    ...
    it('Product Add', () => { //prev
        cy.get('.product-item-info > a.product-item-photo').eq(1).click({force: true});
        cy.get('div').then(($div) => {
            if($div.hasClass('box-tocart')){
                if($div.hasClass('product-options-wrapper')){ // check config product
                    ...
                    cy.screenshot('config product screenshot')
                }
                ...
                if($div.hasClass('bundle-actions')){ // Bundle product add
                    ...
                    cy.screenshot('bundle product screenshot')
                }
   ...
   it('Click on minicart ', () => {
    ...
    cy.screenshot('minicart screenshot')
   })
    //Newsletter
    // it('Newsletter', () =>{
    //     cy.get('div').then(($div) => {
